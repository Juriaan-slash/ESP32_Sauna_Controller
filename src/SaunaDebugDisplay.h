...
private:
  ...
  static constexpr int BODY_Y = HEADER_H + 8;
  ...
  void drawChar(int x, int y, char c, uint16_t color) {
    uint8_t g[5];
    glyph(c, g);
    const bool flipVertical = c >= '0' && c <= '9';

    for (int col = 0; col < 5; ++col) {
      for (int row = 0; row < 7; ++row) {
        const int sourceRow = flipVertical ? 6 - row : row;
  ...
  void drawString(int x, int y, const String& text, uint16_t color) {
    ...
    for (size_t i = 0; i < text.length(); ++i) {
      drawChar(cursor, y, text[i], color);
      cursor += CHAR_W * SCALE;
    }
  }
  ...
  void fillRect(int x, int y, int w, int h, uint16_t color) {
    ...
    const int x1 = min(_width, x + w);
    const int y1 = min(_height, y + h);
  ...
  void drawSection(int x, int y, const String& title) {
    drawString(x, y, title, _headerTextColor);
  }

  void drawField(int x, int y, const String& label, const String& value) {
    drawString(x, y, label, _foreground);
    drawString(x + VALUE_OFFSET, y, value, _foreground);
  }

  void redraw() {
    if (!_buffer || !_lcd) return;

    fillRect(0, 0, _width, _height, _background);
    ...
    drawSection(LEFT_X, y, "SYSTEM");
    y += LINE_HEIGHT;
    drawField(LEFT_X, y, "Board", _systemBoard);      y += LINE_HEIGHT;
    drawField(LEFT_X, y, "Display", _systemDisplay);  y += LINE_HEIGHT;
    drawField(LEFT_X, y, "Touch", _systemTouch);      y += LINE_HEIGHT;
    drawField(LEFT_X, y, "Init time", _initTime);     y += LINE_HEIGHT;
    drawField(LEFT_X, y, "Runtime", _runtime);        y += LINE_HEIGHT + 6;

    drawSection(LEFT_X, y, "SENSORS");
    y += LINE_HEIGHT;
    drawField(LEFT_X, y, "DS2484", _ds2484);          y += LINE_HEIGHT;
    drawField(LEFT_X, y, "K-Type", _kType);           y += LINE_HEIGHT;
    drawField(LEFT_X, y, "CO", _co);                  y += LINE_HEIGHT;
    drawField(LEFT_X, y, "Smoke", _smoke);            y += LINE_HEIGHT + 6;

    drawSection(LEFT_X, y, "SYSTEM LOG");
    y += LINE_HEIGHT;
    drawField(LEFT_X, y, "RGB buffer", _rgbBuffer);   y += LINE_HEIGHT;
    drawField(LEFT_X, y, "Display", _displayLog);     y += LINE_HEIGHT;
    drawField(LEFT_X, y, "USB CDC", _usbCdc);         y += LINE_HEIGHT;
    drawField(LEFT_X, y, "WiFi retries", _wifiReconnects);

    y = BODY_Y;

    drawSection(RIGHT_X, y, "CONNECTION");
    y += LINE_HEIGHT;
    drawField(RIGHT_X, y, "WiFi", _wifi);             y += LINE_HEIGHT;
    drawField(RIGHT_X, y, "SSID", _ssid);             y += LINE_HEIGHT;
    drawField(RIGHT_X, y, "IP", _ip);                 y += LINE_HEIGHT;
    drawField(RIGHT_X, y, "RSSI", _rssi);             y += LINE_HEIGHT;
    drawField(RIGHT_X, y, "RTC", _rtc);               y += LINE_HEIGHT + 6;

    drawSection(RIGHT_X, y, "TEST VALUES");
    y += LINE_HEIGHT;
    drawField(RIGHT_X, y, "Temperature", _temperature); y += LINE_HEIGHT;
    drawField(RIGHT_X, y, "Humidity", _humidity);       y += LINE_HEIGHT;
    drawField(RIGHT_X, y, "Pressure", _pressure);       y += LINE_HEIGHT + 6;

    drawSection(RIGHT_X, y, "HARDWARE");
    y += LINE_HEIGHT;
    drawField(RIGHT_X, y, "PSRAM", _psram);            y += LINE_HEIGHT;
    drawField(RIGHT_X, y, "Free RAM", _freeRam);       y += LINE_HEIGHT;
    drawField(RIGHT_X, y, "Flash", _flash);            y += LINE_HEIGHT;
    drawField(RIGHT_X, y, "CPU", _cpu);
...
public:
  bool begin(LCD* lcd) {
    ...
    _width = _lcd->getFrameWidth();
    _height = _lcd->getFrameHeight();

    _buffer = reinterpret_cast<uint16_t*>(
      heap_caps_malloc(
        _width * _height * sizeof(uint16_t),
        MALLOC_CAP_SPIRAM
      )
    );
  ...
  void setSystem(const String& board, const String& display,
                 const String& touch, const String& initTime,
  ...
  void setConnection(const String& wifi, const String& ssid,
                     const String& ip, const String& rssi,
                     const String& rtc) {
    _wifi = wifi;
    _ssid = ssid;
    _ip = ip;
  ...
  void setSensors(const String& ds2484, const String& kType,
                  const String& co, const String& smoke) {
    _ds2484 = ds2484;
    _kType = kType;
    _co = co;
    _smoke = smoke;
  ...
  void setTestValues(const String& temperature, const String& humidity,
  ...
  void setSystemLog(const String& rgbBuffer, const String& display,
                    const String& usbCdc, const String& wifiReconnects) {
  ...
  void setHardware(const String& psram, const String& freeRam,
                   const String& flash, const String& cpu) {
    _psram = psram;
    _freeRam = freeRam;
    _flash = flash;
    _cpu = cpu;
  ...
  void setDateTime(const String& text, const String& rtcStatus) {
    _dateTime = text;
    _rtc = rtcStatus;
    _dirty = true;
  }

  void setRuntime(const String& text) {
    _runtime = text;
    _dirty = true;
  }
